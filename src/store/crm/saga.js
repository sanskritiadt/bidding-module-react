...
function* getLeads() {
  try {
    ...
    console.log("getCategoier",response.data.data)
    yield put(crmApiResponseSuccess(GET_LEADS, response.data.data));
  } catch (error) {
...
function* onDeleteLead({ payload: lead }) {
  try {
    ...
    yield put(deleteLeadSuccess({ lead, ...response }));
...
function* onAddNewLead({ payload: lead }) {
  try {
    ...
    console.log("onAddNewLead",response.data);
    console.log("onAddNewLead",response.data.data);
