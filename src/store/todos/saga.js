...
function* getTodos() {
  try {
    ...
    console.log("onAddNewTodo",response.data.data)
    yield put(toDoApiResponseSuccess(GET_TODOS, response.data.data));
  } catch (error) {
...
function* onAddNewTodo({ payload: todo }) {
  ...
  try {
    ...
    console.log("onAddNewTodo",response.data.data)
    yield put(addTodoSuccess(response.data.data));
    toast.success("Tag Added Successfully", { autoClose: 3000 });
  } catch (error) {
    yield put(addTodoFail(error));
    toast.error("Tag Added Failed", { autoClose: 3000 });
  }
}
