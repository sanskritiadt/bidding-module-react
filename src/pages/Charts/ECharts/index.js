...
const Echarts = () => {
  ...
  const columns = useMemo(
    ...
    const getAllCountForVehicle = () => {
        ...
        axios.get(`${process.env.REACT_APP_LOCAL_URL_SEQUENCING}/dashboard/getIBSequencingCounts?plantCode=${plantCode}`, config)
        .then(res => {
            const countL = res;
    ...
    const addClusterPlantMap = (index,type) => {
        ...
        try {
            if(type === ''){
                axios.get(`${process.env.REACT_APP_LOCAL_URL_SEQUENCING}/sequencing/getAllSequencingForIBAndOB?flag=IB`, config).then((res) => {
                    const bl = [];
                ...
                axios.get(`${process.env.REACT_APP_LOCAL_URL_SEQUENCING}/sequencing/getAllSequencingByQueue?queueType=${type}&flag=IB`, config).then((res) => {
                    const bl = [];
                    res.map(function(value,index){
                        const av = {};
                        av["Trip Id"] = value.tripId;
    ...
    const handleDownload = async (e) => {
      ...
      const downloadCSV = () => {
        const header = Object.keys(totalDataInVehicle[0]).join(',') + '\n';
        const csv = totalDataInVehicle.map((row) => Object.values(row).join(',')).join('\n');
    ...
    document.title="Sequence Dashboard | EPLMS";
    return (
                        ...
                        <Row>
                            ...
                            <Col lg={6}>
                                <Card>
                                    ...
                                    <CardBody>
                                        <LinewithDataLabels dataColors='["--vz-primary","--vz-success","--vz-warning"]' />
                                    </CardBody>
                                </Card>
                            </Col>
                            <Col lg={4}></Col>
