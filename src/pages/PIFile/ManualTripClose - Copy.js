...
const ManualTripClose = () => {
    ...
    const getStageData = (plantCode) => {debugger;
    ...
    const getAllDeviceData = async (e) => {
        ...
        if (in_put === "") {
            ...
            setErrorInput(false);
            const res = await axios.get(`${process.env.REACT_APP_LOCAL_URL_8082}/manualTripClose/getActiveTrip?plantCode=${Plant_Code}&input=${in_put}`, config)
                .then(res => {
                    if (res.msg) {
                        setCustomer([]);
