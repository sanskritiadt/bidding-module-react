...
// const BidStatusChart = ({ running = 0, completed = 0, notStarted = 0 }) => {
...
//   // Fetch bid status data
//   useEffect(() => {
//     fetchBidStatusData();
//   }, []);

//   const fetchBidStatusData = async () => {
//     try {
//       // Basic auth credentials
//       const credentials = btoa(`${process.env.REACT_APP_API_USER_NAME}:${process.env.REACT_APP_API_PASSWORD}`);

//       // Assuming transporterCode is stored in environment variables or can be retrieved
//       const transporterCode = '916001837'; // Replace with actual code retrieval logic if needed

//       const response = await fetch(`${process.env.REACT_APP_LOCAL_URL_8082}/transporterBidding/getBidStatusByTransporterCode?transporterCode=${transporterCode}`, {
//         method: 'GET',
//         headers: {
//           'Accept': 'application/json',
//           'Content-Type': 'application/json',
...
//       const payload = {
//         id: 0,
//         transBiddingOrdNo: "0", // This should be generated or fetched
//         transAmt: parseFloat(bidAmount),
//         logstAmt: "0", // This should be dynamically determined based on business logic
...
const TransporterDashboard = () => {
  ...
  const fetchFleetEfficiencyData = async () => {
    try {
      ...
      // Assuming transporterCode is stored in environment variables or can be retrieved
      const transporterCode = 'T-000008'; // Replace with actual code retrieval logic if needed
      
      const response = await fetch(`${process.env.REACT_APP_LOCAL_URL_8082}/transporterBidding/getFleetEfficiency?transporterCode=${transporterCode}`, {
        method: 'GET',
        headers: {
          'Accept': '*/*',
          'Content-Type': 'application/json',
  ...
  const fetchDeliveryStatusData = async () => {
    try {
      // Basic auth credentials
      const credentials = btoa(`${process.env.REACT_APP_API_USER_NAME}:${process.env.REACT_APP_API_PASSWORD}`);
      ...
      const response = await fetch(`${process.env.REACT_APP_LOCAL_URL_8082}/transporterBidding/getDeliveryStatus?transporterCode=${transporterCode}`, {
        method: 'GET',
        headers: {
          'Accept': '*/*',
          'Content-Type': 'application/json',
            ...
            <Row className="mb-4">
              ...
              <Col md={4}>
                <DeliveryStatusChart 
                  ...
                  outOfLimit={deliveryStatusData.outOFTimeLimit} 
                />
              </Col>
              <Col md={4}>
                <BidStatusChart 
