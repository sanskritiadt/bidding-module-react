...
const MasterDepartment = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        const res = await axios.put(`${process.env.REACT_APP_LOCAL_URL_8082}/departments/${CurrentID}`, values, config)
        console.log(res.data);
        toast.success("Department Updated Successfully", { autoClose: 3000 });
        getAllDeviceData(Plant_Code);
      }
      else {
        const res = await axios.post(`${process.env.REACT_APP_LOCAL_URL_8082}/departments`, values, config)
        console.log(res);
        if (!res.errorMsg) {
  ...
  const handleCustomerClick = useCallback((arg) => {
    ...
    toggle();
    const id = arg;
    axios.get(`${process.env.REACT_APP_LOCAL_URL_8082}/departments/${id}`, config)
      .then(res => {
        const result = res;
        setValues({
          ...values,
  ...
  const handleDeleteCustomer = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.delete(`${process.env.REACT_APP_LOCAL_URL_8082}/departments/${CurrentID}`, config)
      console.log(res.data);
      getAllDeviceData(Plant_Code);
  ...
  document.title = "Department | EPLMS";
  return (
    <React.Fragment>
      <div className="page-content">
        ...
        />
        <Container fluid>
          <BreadCrumb title={'Department'} pageTitle="Master" />
          <Row>
            <Col lg={12}>
                ...
                <CardHeader className="border-0">
                  <Row className="g-4 align-items-center">
                    <div className="col-sm">
                      <div>
                        <h5 className="card-title mb-0 bg-light">Department Details</h5>
                      </div>
                    </div>
                ...
                <div className="card-body pt-0">
                  <div>
                    ...
                    <TableContainer
                      ...
                      SearchPlaceholder='Search for Department Name or something...'
                    />
                  </div>
                  ...
                  <Modal id="showModal" isOpen={modal} toggle={toggle} centered size="lg">
                    ...
                    <Form className="tablelist-form" onSubmit={handleSubmit}>
                      <ModalBody>
                        <Row className="g-3">
                          <Col md={4}>
                            <Label htmlFor="validationDefault03" className="form-label">Department Code</Label>
                          ...
                          <Col md={4}>
                            <Label htmlFor="validationDefault01" className="form-label">Department Name</Label>
                          ...
                          <Col md={12} className="hstack gap-2 justify-content-end" style={{ marginTop: "45px" }}>
                            ...
                            <button type="submit" className="btn btn-success"> {!!isEdit ? "Update" : "Add Department"} </button>
                          </Col>
