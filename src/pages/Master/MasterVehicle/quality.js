...
const MasterVehicle = () => {
  ...
  const outlineBorderNavtoggle1 = (tab) => {debugger
   ...
   const data12= [{
    "client_id": "rc_ROtVAqxPflQQDYQlzigm",
    "rc_number": "TS01UA9613",
    "fit_up_to": "2024-05-02",
    "registration_date": "2014-05-23",
    "owner_name": "KTC  LOGISTICS",
    "father_name": "MUJAHID KHAN",
    "present_address": "2-5-286/9/3 NEAR INDANE GAS, GOWDON TIRPELLY, ADILABAD, ADILABAD, ADILABAD, ADILABAD, 504001",
    "permanent_address": "2-5-286/9/3 NEAR INDANE GAS, GOWDON TIRPELLY, ADILABAD, ADILABAD, ADILABAD, ADILABAD, 504001",
    "mobile_number": "",
    "vehicle_category": "GCRT",
    "vehicle_chasi_number": "MA1PCALBCE6E47668",
    "vehicle_engine_number": "BE14ZE0265",
    "maker_description": "MAHINDRA TRUXO25 6X2 6C M17 5000 TC 1S CAB BSIII",
    "maker_model": "MAHINDRA & MAHINDRA LTD",
    "body_type": "Open",
    "fuel_type": "DIESEL",
    "color": "RED",
    "norms_type": "",
    ...
    "insurance_upto": "2024-07-01",
    "manufacturing_date": "3/2014",
    "manufacturing_date_formatted": "2014-03",
    "registered_at": "RTA ADILABAD",
    "latest_by": "2024-05-02",
    "less_info": true,
    "tax_upto": "2024-06-30",
    "tax_paid_upto": "2024-06-30",
    "cubic_capacity": "7118.00",
    "vehicle_gross_weight": "28000.00",
    "no_cylinders": "6",
    "seat_capacity": "4",
    "unladen_weight": "8900.00",
    "vehicle_category_description": "GOODS CARRIAGE",
    "permit_number": "Adad121",
    "rc_status": "ACTIVE",
    "masked_name": false
},]

  const SearchVahaan = async () =>{debugger
    var vehicleNo = document.getElementById('VehicleVahaan').value
    var json ={
      'id_number':vehicleNo
    }
    try{
      await axios.post(`${process.env.REACT_APP_LOCAL_URL_8082}/eplmsVahan/getVahanInfo`,json, config)
      .then(res => {
        const plant = res;
        const rcNumberExists = VahanData.some(item => item.rc_number === vehicleNo);
        if(!rcNumberExists){
          setVahanData([...VahanData,...[plant]]);
        }
        else{
          setVahanData(VahanData);
        }
  ...
  const columnVahaan = useMemo(
    () => [
      ...
      {
        Header: "Vehicle Chasi Number",
        accessor: "vehicle_chasi_number",
        filterable: false,
      },
      {
        Header: "Vehicle Engine Number",
        accessor: "vehicle_engine_number",
        fixed: 'left',
      },
      {
        Header: "Body Type",
      ...
      {
        Header: "Fitness Up To",
        accessor: "fit_up_to",
        Cell: ({ row }) => {
          const registrationDate = new Date(row.original.fit_up_to);
          const currentDate = new Date();
          // const datePortion = currentDate.toISOString().split('T')[0];
          ...
          if (registrationDate < currentDate) {
            ...
            return <span style={{color:'#2aef2a'}}> {formattedDate}  </span>;
          } else {
      ...
      {
        Header: "Unladen Weight",
        accessor: "unladen_weight",
        fixed: 'left',
      },
      {
        Header: "Insurance Upto",
        accessor: "insurance_upto",
        Cell: ({ row }) => {
          const registrationDate = new Date(row.original.insurance_upto);
          const currentDate = new Date();
          // const datePortion = currentDate.toISOString().split('T')[0];
          const formattedDate = `${String(registrationDate.getDate()).padStart(2, '0')}-${String(registrationDate.getMonth() + 1).padStart(2, '0')}-${registrationDate.getFullYear()}`;
      ...
      },
      {
        ...
        accessor: "tax_upto",
        Cell: ({ row }) => {
          const registrationDate = new Date(row.original.tax_upto);
          const currentDate = new Date();
          // const datePortion = currentDate.toISOString().split('T')[0];
          const formattedDate = `${String(registrationDate.getDate()).padStart(2, '0')}-${String(registrationDate.getMonth() + 1).padStart(2, '0')}-${registrationDate.getFullYear()}`;
      ...
      },
      {
        ...
        Cell: (cell) => {
          switch (cell.value) {
            ...
            default:
              return <span className="badge text-uppercase badge-soft-info"> </span>;
          }
        },
        fixed: 'left',
      }
      ...
      },
      {
        ...
        Cell: (cell) => {
          switch (cell.value) {
            ...
            default:
              return <span className="badge text-uppercase badge-soft-info"> No </span>;
          }
        }
      },
      {
              ...
              <Card id="customerList">
                <div className="card-body pt-4">
                  ...
                  <Nav pills className="nav-customs nav-danger mb-3 nav nav-pills">
                    <NavItem>
                      <NavLink id="tab1" style={{ cursor: "pointer" }} className={classnames({ active: outlineBorderNav1 === "1", })} onClick={() => { outlineBorderNavtoggle1("1"); }}>  Vehicle Details     </NavLink>
                    </NavItem>
                    <NavItem>
                      <NavLink id="tab2" style={{ cursor: "pointer" }} className={classnames({ active: outlineBorderNav1 === "2", })} onClick={() => { outlineBorderNavtoggle1("2"); }}>   Vahaan Details     </NavLink>
                    </NavItem>
                  </Nav>

                  <TabContent activeTab={outlineBorderNav1} className="text-muted">
                    <TabPane tabId="1" id="border-nav-home" className="tab-pane shadow_light" style={{marginTop:"-15px"}}>
                  ...
                  <TabContent activeTab={outlineBorderNav1} className="text-muted">
                    <TabPane tabId="2" id="border-nav-home" className="tab-pane shadow_light" style={{marginTop:"-15px"}}>
                      ...
                      <CardHeader className="border-0">
                        <Row className="g-4 align-items-center mb-5">
                              ...
                              <Col md={3}>
                                <Label htmlFor="validationDefault01" className="form-label" style={{fontWeight:'bold'}}>Vehicle Number</Label>
                              ...
                              <Col lg={3} >
                            <div>
                              <button type="button" className="btn btn-success justify-content-end" style={{ marginTop: "28px", width: "auto", marginLeft: "10px" }} onClick={()=>{SearchVahaan()}}> Search</button>
                            </div>
                          </Col>
                        </Row>
                      </CardHeader>
                      <div className="card-body pt-0">
                        <div>
                          {/* {VahanData && VahanData.length ? ( */}
                            <TableContainer
                              columns={columnVahaan}
                              data={VahanData}
                              isGlobalFilter={true}
                              isAddUserList={false}
                              customPageSize={5}
