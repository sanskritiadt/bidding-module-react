...
const MasterToleranceBLK = () => {
  ...
  const getAllDeviceData = (plantcode) => {
    ...
    axios.get(`${process.env.REACT_APP_LOCAL_URL_8082}/api/tolerances/getTolaranceByPlantCode/${plantcode}`, config)
      .then(res => {
        const device = res;
        setDevice(device);
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        const res = await axios.put(`${process.env.REACT_APP_LOCAL_URL_8082}/api/tolerances/updateTolerance/${CurrentID}`, values, config)
        console.log(res);
        toast.success("Tolerance Updated Successfully", { autoClose: 3000 });
        getAllDeviceData(Plant_Code);
      }
      else {
        const res = await axios.post(`${process.env.REACT_APP_LOCAL_URL_8082}/api/tolerances/createTolarance`, values, config)
        console.log(res);
        if (!res.errorMsg) {
  ...
  const handleCustomerClick = useCallback((arg) => {
    ...
    toggle();
    const id = arg;
    axios.get(`${process.env.REACT_APP_LOCAL_URL_8082}/api/tolerances/getTolaranceById/${id}`, config)
      .then(res => {
        const result = res;
        setValues({
          ...values,
  ...
  const handleDeleteCustomer = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.delete(`${process.env.REACT_APP_LOCAL_URL_8082}/api/tolerances/deleteToleranceById/${CurrentID}`, config)
  ...
  document.title = "Tolerance BLK | EPLMS";
  return (
    <React.Fragment>
      <div className="page-content">
        ...
        />
        <Container fluid>
          <BreadCrumb title={'Tolerance Master Bulker'} pageTitle="Master" />
          <Row>
            <Col lg={12}>
                ...
                <CardHeader className="border-0">
                  <Row className="g-4 align-items-center">
                    <div className="col-sm">
                      <div>
                        <h5 className="card-title mb-0 bg-light">Bulker Tolerance Details</h5>
                      </div>
                    </div>
...
export default MasterToleranceBLK;
