...
//   );
// };
// console.log("Session storage auth user:", JSON.parse(sessionStorage.getItem("authUser")));
...
//           `${process.env.REACT_APP_LOCAL_URL_8082 || 'http://localhost:8085'}/routes?plantCode=${plantCode}`,
//           axiosConfig
//         );
...
//       if (value === 'All') {
//         // Use plantcode from session storage instead of hardcoded 'NE25'
//         fetchTransportersByFlag('A', plantcode);
//       }
...
//         } else {
//           toast.info("Please select a from location first to see plant-based transporters.", { autoClose: 3000 });
//         }
//       }
//     }
...
//           `${process.env.REACT_APP_LOCAL_URL_8082 || 'http://localhost:8085'}/locations/filterLocation/a`,
//           config
//         );
...
//       const response = await fetch(`${process.env.REACT_APP_LOCAL_URL_8082 || 'http://localhost:8085'}/biddingMaster/bulk`, {
//         method: 'POST',
//         headers: getAuthHeaders(),
...
//                   display: "flex",
//                   alignItems: "center",
//                   width: "100%" // Added width for consistency
//                 }}>
//                   <div style={{
//                     flex: 1,
//                     position: "relative",
//                     minHeight: "38px" // Added minimum height to ensure consistent size
//                   }}>
//                     <div
//                       className="bulk-order-transporter-selector"
//                       onClick={() => setShowTransporterDropdown(!showTransporterDropdown)}
...
//                     )}
//                   </div>
//                   {/* Eye icon container - always visible */}
//                   <span
...
//                       width: "28px",
//                       height: "28px",
//                       minWidth: "28px", // Ensure fixed width
...
//         {/* All step conditionals above */}

//         {/* Transporter modal - available in all steps */}
...
//                     <i className="ri-inbox-line" style={{ fontSize: "24px" }}></i>
//                     <div style={{ marginTop: "8px" }}>No transporters selected</div>
//                   </div>
//                 )}
//               </div>
//               <div
...
const BulkOrder = ({ bidNo }) => {
  ...
  useEffect(() => {
    ...
    const fetchMaterials = async () => {
      try {
        const rawResponse = await axios.get(
          `${process.env.REACT_APP_LOCAL_URL_8085 || 'http://localhost:8085'}/materials?plantCode=${plantCode}`,
          config
        );
        console.log("RAW API Response:", rawResponse);
  ...
  useEffect(() => {
    const fetchCities = async () => {
      ...
      try {
        const rawResponse = await axios.get(
          `${process.env.REACT_APP_LOCAL_URL_8085 || 'http://localhost:8085'}/locations/filterLocation/a`,
          config
        );
