...
import YardGraphDataIB from "./yardGraphDataIB";


const YardDashboardIB = () => {
    ...
    const getCountData = async () => {
        ...
        let plantcode = obj.data.plantCode;
        try {
            const res = await axios.get(`${process.env.REACT_APP_LOCAL_URL_8082}/ib-yard-in/totalYardInCounts/${plantcode}`, config)
                .then(res => {
                    const data = res;
                    setCountData(data);
                    setloader(false);
    ...
    const RefreshedModalData = async (cardHeader) => {
        if (cardHeader === "Vehicle Yard In Details" && currentBodyType === "BAG") {
            setloaderModal(true);
            try {
                const res = await axios.get(`${process.env.REACT_APP_LOCAL_URL_8082}/ib-yard-in/vehicle?plantCode=${plant_Code}`, config);
                const data = res;
        ...
        if (cardHeader === "PO Mapped Details" && currentBodyType === "BAG") {
            setloaderModal(true);
            try {
                const res = await axios.get(`${process.env.REACT_APP_LOCAL_URL_8082}/ib-yard-in/poMapped?plantCode=${plant_Code}`, config);
                const data = res;
        ...
        if (cardHeader === "Without PO Mapped Details" && currentBodyType === "BAG") {
            setloaderModal(true);
            try {
                const res = await axios.get(`${process.env.REACT_APP_LOCAL_URL_8082}/ib-yard-in/withoutPoMapped?plantCode=${plant_Code}`, config);
                const data = res;
        ...
        //     setloaderModal(true);
        //     try {
        //         const res = await axios.get(`${process.env.REACT_APP_LOCAL_URL_8082}/ib-yard-in/documentExpired?plantCode=${plant_Code}`, config);
        //         const data = res;
        ...
        //     setloaderModal(true);
        //     try {
        //         const res = await axios.get(`${process.env.REACT_APP_LOCAL_URL_8082}/ib-yard-in/documentExpired?plantCode=${plant_Code}&vehicleBodyType=BLK`, config);
        //         const data = res;
        //         setDocExpiredData(data);
        //         setLatestData(data);
        //         setloaderModal(false);
    ...
    const getVehicleYardInData = async (bodyType) => {
        ...
        if (bodyType === "BAG") {
            ...
            setloader(true);
            try {
                const res = await axios.get(`${process.env.REACT_APP_LOCAL_URL_8082}/ib-yard-in/vehicle?plantCode=${plant_Code}&vehicleBodyType=BLK`, config);
                const data = res;
                setVehicleYardInData(data);
                setLatestData(data);
                setViewModal();
    ...
    const openModal = (data, bodyType) => {
        ...
        if (data === 2) {
            setCardHeader("PO Mapped Details");
            getDIReceivedData(bodyType);
            setActiveTable(data);
        }
        if (data === 3) {
            setCardHeader("Without PO Mapped Details");
    ...
    document.title = "Yard Dashboard IB | EPLMS";
    return (
        <React.Fragment>
            <div className="page-content">
                <Container fluid>
                    <BreadCrumb title="Yard Dashboard IB" pageTitle="EPLMS" />
                    <Row>
                        <Col lg={12}>
                                ...
                                <div className="card-body pt-4">
                                    ...
                                    <TabContent activeTab={outlineBorderNav} className="text-muted">
                                        <TabPane tabId="1" id="border-nav-home">
                                                        ...
                                                        <CardBody>
                                                            <div className="d-flex">
                                                                <div className="flex-grow-1">
                                                                    ...
                                                                    <button class="btn btn-outline-primary btn-border btn-sm yrd_btn" onClick={() => { openModal(1, "BAG") }} style={{ marginTop: "-4px" }}><b>Total Vehicle || {countData.vehicleYardIn}</b></button>
                                                                </div>
                                                ...
                                                <Col xl={3}>
                                                    <Card className="shadow bg-success-subtle">
                                                        <CardBody >
                                                            <div className="d-flex">
                                                                <div className="flex-grow-1">
                                                                    <h6 className="text-muted mb-3">PO Mapped</h6>
                                                                    {/* <h4 className="mb-0">{countData.diReceived}</h4> */}
                                                                    <button class="btn btn-outline-primary btn-border btn-sm yrd_btn" onClick={() => { openModal(2, "BAG") }} style={{ marginTop: "-4px" }}><b>PO Mapped || {countData.poMapped}</b></button>
                                                                </div>
                                                                <div className="flex-shrink-0 avatar-sm">
                                                                    <div className={"avatar-title fs-22 rounded bg-success"}>
                                                ...
                                                <Col xl={3}>
                                                    <Card className="shadow bg-warning-subtle">
                                                        <CardBody >
                                                            <div className="d-flex">
                                                                <div className="flex-grow-1">
                                                                    <h6 className="text-muted mb-3">PO Unmapped</h6>
                                                                    {/* <h4 className="mb-0">{countData.withoutDiReceived}</h4> */}
                                                                    <button class="btn btn-outline-primary btn-border btn-sm yrd_btn" onClick={() => { openModal(3, "BAG") }} style={{ marginTop: "-4px" }}><b>PO Unmapped || {countData.withoutPoMapped}</b></button>
                                                                </div>
                                                                <div className="flex-shrink-0 avatar-sm">
                                                        ...
                                                        <CardBody >
                                                            <div className="d-flex">
                                                                <div className="flex-grow-1">
                                                                    ...
                                                                    <button class="btn btn-outline-primary btn-border btn-sm yrd_btn" onClick={() => { openModal(4, "BAG") }} style={{ marginTop: "-4px" }}><b>Vehicle Doc Exp. || {countData.vehicleDocumentExp}</b></button>
                                                                </div>
                                                                <div className="flex-shrink-0 avatar-sm">
                                            ...
                                            <YardGraphDataIB key={refreshKey} />
...
export default YardDashboardIB;
