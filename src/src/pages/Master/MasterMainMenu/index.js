...
import '../MasterMainMenu/MainMenu.css';
...
const MasterMainMenu = () => {
  ...
  const getAllDeviceData = () => {
    ...
    axios.get(`${process.env.REACT_APP_LOCAL_URL_8082}/main-menu`, config)
      .then(res => {
        const device = res;
        setDevice(device);
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        const res = await axios.put(`${process.env.REACT_APP_LOCAL_URL_8082}/main-menu/${CurrentID}`, values, config)
        console.log(res);
        toast.success("Main Menu Updated Successfully", { autoClose: 3000 });
        getAllDeviceData();
      }
      else {
        const res = await axios.post(`${process.env.REACT_APP_LOCAL_URL_8082}/main-menu`, values, config)
        console.log(res);
        if (!res.errorMsg) {
          toast.success("Main Menu Added Successfully.", { autoClose: 3000 });
        }
        else {
  ...
  const handleCustomerClick = useCallback((arg) => {
    ...
    toggle();
    const id = arg;
    axios.get(`${process.env.REACT_APP_LOCAL_URL_8082}/main-menu/${id}`, config)
      .then(res => {
        const result = res;
        setValues({
          ...values,
  ...
  const handleDeleteCustomer = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.delete(`${process.env.REACT_APP_LOCAL_URL_8082}/main-menu/${CurrentID}`, config)
      console.log(res.data);
      getAllDeviceData();
      toast.success("Main Menu Deleted Successfully", { autoClose: 3000 });
  ...
  document.title = "Main Menu | EPLMS";
  return (
    <React.Fragment>
                ...
                <CardHeader className="border-0">
                  <Row className="g-4 align-items-center">
                    <div className="col-sm">
                      <div>
                        <h5 className="card-title mb-0 bg-light">Main Menu Details</h5>
                      </div>
                    </div>
                    <div className="col-sm-auto">
                      <div>
                        ...
                        >
                          <i className="ri-add-line align-bottom me-1"></i> Add Main Menu
                        </button>{" "}
                ...
                <div className="card-body pt-0">
                  <div>
                    ...
                    <TableContainer
                      ...
                      SearchPlaceholder='Search for Main Menu Name or something...'
                    />
                  </div>
                  ...
                  <Modal id="showModal" isOpen={modal} toggle={toggle} centered size="lg">
                    <ModalHeader className="bg-light p-3" toggle={toggle}>
                      {!!isEdit ? "Edit Main Menu" : "Add Main Menu"}
                    </ModalHeader>
                    <Form className="tablelist-form" onSubmit={handleSubmit}>
                      <ModalBody>
                        ...
                        <Col md={8} className="hstack gap-2 justify-content-end" style={{ marginTop: "45px" }}>
                          ...
                          <button type="submit" className="btn btn-success"> {!!isEdit ? "Update" : "Add Main Menu"} </button>
                        </Col>
                        </Row>
                      </ModalBody>
                      <ModalFooter>
