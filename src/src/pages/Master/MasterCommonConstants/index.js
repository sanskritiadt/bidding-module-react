...
import '../MasterCommonConstants/CommonConstants.css';
...
const MasterCommonConstants = () => {
  ...
  const getAllDeviceData = (plantcode) => {
    ...
    axios.get(`${process.env.REACT_APP_LOCAL_URL_8082}/common-constants?plantCode=${plantcode}`, config)
      .then(res => {
        const device = res;
        setDevice(device);
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        const res = await axios.put(`${process.env.REACT_APP_LOCAL_URL_8082}/common-constants/${CurrentID}`, values, config)
        console.log(res);
        toast.success("Common Constant Updated Successfully", { autoClose: 3000 });
        getAllDeviceData(Plant_Code);
      }
      else {
        const res = await axios.post(`${process.env.REACT_APP_LOCAL_URL_8082}/common-constants`, values, config)
        console.log(res);
        if (!res.errorMsg) {
          toast.success("Common Constant Added Successfully.", { autoClose: 3000 });
        }
        else {
  ...
  const handleCustomerClick = useCallback((arg) => {
    ...
    toggle();
    const id = arg;
    axios.get(`${process.env.REACT_APP_LOCAL_URL_8082}/common-constants/${id}`, config)
      .then(res => {
        const result = res;
        setValues({
          ...values,
  ...
  const handleDeleteCustomer = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.delete(`${process.env.REACT_APP_LOCAL_URL_8082}/common-constants/${CurrentID}`, config)
      console.log(res.data);
      getAllDeviceData(Plant_Code);
      toast.success("Common Constant Deleted Successfully", { autoClose: 3000 });
  ...
  document.title = "Common Constants | EPLMS";
  return (
    <React.Fragment>
      <div className="page-content">
        ...
        />
        <Container fluid>
          <BreadCrumb title={'Common Constants'} pageTitle="Master" />
          <Row>
            <Col lg={12}>
                ...
                <CardHeader className="border-0">
                  <Row className="g-4 align-items-center">
                    <div className="col-sm">
                      <div >
                        <h5 className="card-title mb-0 bg-light">Common Constants Details</h5>
                      </div>
                    </div>
                    <div className="col-sm-auto">
                      <div className="top_btn">
                        ...
                        >
                          <i className="ri-add-line align-bottom me-1"></i> Add Common Constants
                        </button>{" "}
                ...
                <div className="card-body pt-0">
                  ...
                  <Modal id="showModal" isOpen={modal} toggle={toggle} centered size="lg">
                    <ModalHeader className="bg-light p-3" toggle={toggle}>
                      {!!isEdit ? "Edit Common Constants" : "Add Common Constants"}
                    </ModalHeader>
                    <Form className="tablelist-form" onSubmit={handleSubmit}>
                      <ModalBody>
                        <Row className="g-3">
                          ...
                          <Col md={4}>
                            <Label htmlFor="validationDefault04" className="form-label">Value</Label>
                            <Input type="text" required className="form-control"
                          ...
                          <Col md={4}>
                            <Label htmlFor="validationDefault03" className="form-label">Description</Label>
                          ...
                          <Col md={isEdit ? 8 : 12} className="hstack gap-2 justify-content-end" style={{ marginTop: "45px" }}>
                            ...
                            <button type="submit" className="btn btn-success"> {!!isEdit ? "Update" : "Add Common Constants"} </button>
                          </Col>
                        </Row>
                      </ModalBody>
                      <ModalFooter>
...
export default MasterCommonConstants;
