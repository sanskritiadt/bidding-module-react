...
import '../MasterDeviceType/DeviceType.css';
...
const MasterDeviceType = () => {
  ...
  const getAllDeviceData = () => {
    ...
    axios.get(`${process.env.REACT_APP_LOCAL_URL_8082}/device-type`, config)
      .then(res => {
        const device = res;
        setDevice(device);
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        const res = await axios.put(`${process.env.REACT_APP_LOCAL_URL_8082}/device-type/${CurrentID}`, values, config)
        console.log(res.data);
        toast.success("Device Type Updated Successfully", { autoClose: 3000 });
        getAllDeviceData();
      }
      else {
        const res = await axios.post(`${process.env.REACT_APP_LOCAL_URL_8082}/device-type`, values, config)
        console.log(res);
        if (!res.errorMsg) {
          toast.success("Device Type Added Successfully.", { autoClose: 3000 });
        }
        else {
  ...
  const handleCustomerClick = useCallback((arg) => {
    ...
    toggle();
    const id = arg;
    axios.get(`${process.env.REACT_APP_LOCAL_URL_8082}/device-type/${id}`, config)
      .then(res => {
        const result = res;
        setValues({
          ...values,
          ...
          "commandFormat": result.commandFormat,
  ...
  const handleDeleteCustomer = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.delete(`${process.env.REACT_APP_LOCAL_URL_8082}/device-type/${CurrentID}`, config)
      console.log(res.data);
      getAllDeviceData();
      toast.success("Device Type Deleted Successfully", { autoClose: 3000 });
  ...
  document.title = "Device Type | EPLMS";
  return (
    <React.Fragment>
                ...
                <CardHeader className="border-0">
                  <Row className="g-4 align-items-center">
                    <div className="col-sm">
                      <div>
                        <h5 className="card-title mb-0 bg-light">Device Type Details</h5>
                      </div>
                    </div>
                    <div className="col-sm-auto">
                      <div>
                        ...
                        >
                          <i className="ri-add-line align-bottom me-1"></i> Add Device Type
                        </button>{" "}
                ...
                <div className="card-body pt-0">
                  ...
                  <Modal id="showModal" isOpen={modal} toggle={toggle} centered size="lg">
                    <ModalHeader className="bg-light p-3" toggle={toggle}>
                      {!!isEdit ? "Edit Device Type" : "Add Device Type"}
                    </ModalHeader>
                    <Form className="tablelist-form" onSubmit={handleSubmit}>
                      <ModalBody>
                        <Row className="g-3">
                          ...
                          <Col md={4}>
                            <Label htmlFor="validationDefault04" className="form-label">Command Format</Label>
                            <Input type="text" required className="form-control"
                              id="validationDefault04"
                              name="commandFormat"
                              placeholder="Enter Command Format"
                              value={values.commandFormat}
                              onChange={handleInputChange}
                            />
                          </Col>
                          <Col md={4}>
                            <Label htmlFor="validationDefault04" className="form-label">Device Group</Label>
                            <Input type="text" required className="form-control"
                              id="validationDefault04"
                              name="deviceGroup"
                          ...
                          <Col md={8} className="hstack gap-2 justify-content-end" style={{ marginTop: "45px" }}>
                            <button type="button" className="btn btn-light" onClick={toggle}> Close </button>
                            <button type="submit" className="btn btn-success"> {!!isEdit ? "Update" : "Add Device Type"} </button>
                          </Col>
