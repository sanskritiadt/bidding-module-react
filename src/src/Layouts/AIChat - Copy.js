...
import { Container, Row, Col, Card, CardHeader, Modal, Form, ModalBody, ModalFooter, Offcanvas, OffcanvasBody, ModalHeader, Label, Input, FormFeedback, Nav, NavItem, NavLink, TabContent, TabPane, CardBody } from "reactstrap";
    ...
    const handleSendMessage = async (e) => {
        ...
        if (inputText.trim() !== '') {
            ...
            try {
                const res = await axios.post(`http://localhost:8080/http://192.168.0.27:5000/chat`, values);
                console.log(res);
                setMessages([...messages, { text: inputText, sender: 'User', finalText: res.reply, responseDateTimeDate: getFormattedDateTime(), triggerDateTime: getFormattedDateTime() }]);
                scrollToBottom();
    ...
    return (
        <React.Fragment>
            ...
            >
                ...
                >
                    <h5 className="fs-16 m-0" id="hd_color">
                        <img style={{ marginTop: "-4px" }} src={Ai_Icon} alt="" height="40" />
                        FIRLO AI
                    </h5>
                    <i
                        className={`${isExpanded ? "ri-subtract-line" : "ri-arrow-right-up-line"} fs-16 position-absolute top-0 end-0 m-2 text-white`}
                        style={{ cursor: "pointer" }}
                        onClick={toggleModalSize}
                    ></i>
                </ModalHeader>
                <div className="chatbot-container" style={{ backgroundColor: "#f9f9f9" }}>
                    <div className={`chatbot-messages ${isExpanded ? 'chatbot-messages-expanded' : ''}`} ref={ref}>
                    ...
                    <div className="chatbot-input" style={{ marginTop: "5px", padding: "0 5px", marginBottom: "5px" }}>
                        <Form className="tablelist-form" onSubmit={handleSendMessage} style={{ display: "flex", alignItems: "center", position: "relative" }}>
                            <input
                                ...
                                style={{ width: isExpanded ? "-webkit-fill-available" : "16rem", padding: "0 40px 0 15px", borderRadius: "30px", border: "1px solid #ddd", boxShadow: "0 2px 5px rgba(0, 0, 0, 0.1)", outline: "none", height: "40px" }}
                            />
                            <Link to="#" className="btn btn-link link-success btn-lg position-absolute" onClick={(event) => { toggleOffCanvas(); startListening(event) }} style={{ right: "44px", top: "-5px" }}>
